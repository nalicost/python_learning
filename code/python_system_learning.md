...
## Linux操作系统及其组成
1. Linux操作系统：os
   * 作用：管理计算机硬件与软件资源的程序，可简单理解为将计算机硬件的功能封装器起来，向外提供必要的接口让软件调用，以此实现软件功能，是联系软件与硬件的桥梁
2. Linux操作系统组成
...
4. Linux常用命令
   ...
   * ```ls```：展示目录内容（直接写即可）
     ...
     * ```ls -l```: 查看文件目录下各个文件属性
        ...
        * 第六七八列表示文件创建时间
        * 最后一列为文件名称
   * ```cp -r```：复制文件（cp与-r之间加入需复制的文件或目录，-r后加入在当前目录中要放入的子目录）
   * ```mv```：移动文件（空格加上移动文件或目录名称在空格加上要移动至的目录即可）
   * ```rm```：删除文件（空格加文件名即可）
...
注：客户端与服务端的套接字类型要匹配，同时recv与send要与双方相互配合，防止阻塞，服务端要注意异常处理，防止异常退出（服务端一般是不关闭的，以提供稳定的服务）
###### TCP套接字数据传输的特点
...
* 原理：创建多个进程，当事件处理完，不退出进程二是接取下个事件，直到事件处理完，一并销毁。
* 语法
  * ```pool = Pool(process)```
    * 作用：创建进程池并创建进程池中进程数量
    * 参数
    ...
    * 代码实现
      * Value实现 
        1. ```obj = Value(ctype, data)```
           * 作用：开辟共享空间
           * 参数
             1. ctype：表示共享内存空间传的数据类型（与struct相同）
             2. data：共享空间的初始数据
           * 返回值
             1. obj：共享内存对象    
        2. ```obj.value```
...
#### python线程GIL
##### GIL的作用
python解释器设计了解释器锁，即GIL（全局解释器锁），使得python本体线程能够有序与系统交互，保护python解释器免受所现称并发争抢资源的影响（共享资源的不一致修改）
##### GIL的特点
